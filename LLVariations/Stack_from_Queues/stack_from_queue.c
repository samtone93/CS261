/* CS261- Assignment 3 - stack_from_queue.c*/
/* Name: Samantha Tone (tones@oregonstate.edu)
 * Date: 4/30/18
 * Solution description: implements a stack using 2 queues
 */
...
void listQueueInit(struct listQueue *q) {
    ...
    // assigns first & last links to sentinel
...
struct listQueue * listQueueCreate()
{
    ...
    // uses initialization to set values
    listQueueInit(q);
...
void listQueueAddBack(struct listQueue *q, TYPE e) {
    ...
    // puts it at the lastLink position but must assign current last link next before if not null
    if (q->lastLink!=0) {
        q->lastLink->next = new;
    }
    q->lastLink = new;

    // if we don't have a firstLink
    if (!q->firstLink) {
        q->firstLink = new;
    }
...
void listQueueRemoveFront(struct listQueue *q) {
    ...
    // store elements of front value then move firstLink
    struct link* front = q->firstLink;
    q->firstLink = front->next;

    // if removed front is the lastLink switch it to NULL
    if (q->lastLink == front) {
        q->lastLink = NULL;
    }
...
TYPE listQueueFront(struct listQueue *q) {
    // checks queue is not empty
    assert(!listQueueIsEmpty(q));
    // returns value of firstLink
...
struct linkedListStack * linkedListStackCreate(){
    ...
    // calls initialization function to store values
...
void linkedListStackPop(struct linkedListStack *s) {
    if (linkedListStackIsEmpty(s)) {
        printf("Stack is currently empty; cannot pop value\n\n");
        return;
    }
