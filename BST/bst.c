...
{
    // asserts that precondition is met
    assert(val != NULL);

    // case where we can add node is empty tree
    if (cur == 0) {
    ...
    // recur down left is compare returns val <= cur->val
    else if (compare(val, cur->val) <= 0) {
        cur->left = _addNode(cur->left, val);
    }

    // recur down right if compare returns val > cur->val
    else {
        cur->right = _addNode(cur->right, val);
    }

    // returns to root node once new is added to appropriate node
    return cur;
}
...
{
    // asserts tree & val are both not NULL
    assert (tree != NULL);
    assert (val != NULL);

    // initializes searchNode as root
    struct Node* searchNode = tree->root;

    // while loop traverses tree using compare until it hits a NULL node
    while (searchNode != NULL) {
        // if val < searchNode->val traverse left
        if (compare(val, searchNode->val) == -1) {
            searchNode = searchNode->left;
        }

        // if val > searchNode->val traverse right
        else if (compare(val, searchNode->val) == 1) {
    ...
    // if while loop is excited val is not present
    return 0;
}
...
{
    // asserts cur is not NULL and sets up node for traversing
    assert (cur != NULL);
    struct Node* leftOracle = cur;

    // uses while loop to traverse entire left side
    while (leftOracle->left != NULL) {
        leftOracle = leftOracle->left;
    }

    // return the value of the final leftOracle
    return leftOracle->val;
}
...
{
    // asserts cur is not NULL
    assert (cur != NULL);

    // recursive case to traverse the left side
    if (cur->left != NULL) {
        cur->left = _removeLeftMost(cur->left);
        return cur;
    }

    // once it encounters the left most value, the curr is freed
    else {
        struct Node* curHold = cur->right;
        free(cur);
        return curHold;
    }
}
...
{
    // asserts the val & curr aren't null (contains is determined by traversing)
    assert(val != NULL);
    assert(cur != NULL);

    // go left if val < cur->val
    if (compare(val, cur->val) == -1) {
        cur->left = _removeNode(cur->left, val);
    }

    // go right if val > cur->val
    else if (compare(val, cur->val) == 1) {
        cur->right = _removeNode(cur->right, val);
    }

    // series of additional conditions once val is found to adjust tree
    else {
        // cur has no child
        ...
        // cur has only left child
        else if (cur->right == NULL && cur->left != NULL) {
            struct Node* curHold = cur->left;
            free(cur);
            return curHold;
        }

        // cur has only right child
